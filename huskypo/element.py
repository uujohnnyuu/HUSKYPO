...
class Element:
    ...
    def __init__(
        ...
        self.by = by
        if self.by is not None and self.by not in ByAttribute.VALUES:
            raise ValueError(f'The locator strategy "{self.by}" is undefined.')
    ...
    def is_viewable(self, timeout: int | float | None) -> bool:
        """
        Appium API.
        ...
        - timeout: Maximum time in seconds to wait for the element to become present.
        """
        present = self.is_present(timeout)
        status = present
        if present:
    ...
    def swipe_into_view(
        ...
        # Get border.
        border = self._get_swipe_border(direction, border)

        # Determine v or h, and actual swiping range.
        coordinate = self._get_swipe_coordinate(direction, *border, start, end, fix)

        # Start swiping and check whether it is viewable in max count of swiping.
        viewable = self._start_swiping(*coordinate, duration, timeout, max_swipe)

        # Start adjusting when element is viewable.
        self._start_swipe_adjusting(viewable, *border, *coordinate, max_adjust, min_distance, duration)

        # Return self to re-trigger the element finding process, thereby avoiding staleness issues.
        return self

    def _get_swipe_border(
            self,
            direction: str,
    ...
    def _get_swipe_coordinate(
        ...
        if 'v' in direction.lower():
            ...
            raise ValueError('Parameter "dirtype" should be "v", "va", "h" or "ha".')
