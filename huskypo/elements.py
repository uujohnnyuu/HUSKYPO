...
class Elements:
    ...
    def find(
        ...
        if index is not None:
            try:
                ...
                # Catch and reraise IndexError when elements are present but index is out of range.
                raise
        return elements
    ...
    def wait_all_not_visible(
        ...
        """
        try:
            # Since both not visible and not present results in an empty list,
            # there is no need to evaluate the return result of until_not in this case.
            # TODO: using ec_extension to rebuild to distinguish these status.
            self.wait(timeout).until_not(
                ec.visibility_of_any_elements_located(self.locator),
    ...
    def quantity(self):
        ...
        """
        try:
            return len(self.wait_all_present())
        except TimeoutException:
            return 0
